...
function ColorRow(props: { label: string; value: string; onChange: (next: string) => void }) {
  ...
  return (
    <div className="mt-2 flex items-center gap-2">
      ...
      />
      <span className="w-9 shrink-0 text-right font-mono text-xs text-popover-foreground">
        {pct}%
      </span>
    </div>
  );
      ...
      <div className="flex min-h-0 flex-1">
        ...
        >
          ...
          <TabsPrimitive.Content
            value="term"
            className="min-h-0 min-w-0 flex-1 overflow-y-auto scrollbar-thin pl-3"
          >
            {/* ── Font size ──────────────────────────────────────────── */}
            <div className={section}>
              <div className="flex items-center justify-between">
            ...
            <div className={`${section} mt-4`}>
              ...
              <div className="mt-2 flex items-center gap-2">
                <button
                  ...
                  onClick={() =>
                    setLineHeightDefault(clampLineHeight(lineHeightDefault - LINE_HEIGHT_STEP))
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setLineHeightDefault(clampLineHeight(Number(e.target.value) / 10))
                  }
              ...
              <p className={sectionHint}>
                {t('settings.lineHeightHint', { default: lineHeightDefault.toFixed(1) })}
              </p>
            </div>
            ...
            <div className={`${section} mt-4`}>
              ...
              {openGroups.core && (
                <>
                  ...
                  />
                  <ColorRow
                    label={t('settings.cursorAccent')}
                    value={termTheme.cursorAccent}
                    onChange={setThemeColor('cursorAccent')}
                  />
                </>
              )}
            </div>
            ...
            <div className={`${section} mt-4`}>
              ...
              {openGroups.selection && (
                <>
                  <ColorRow
                    label={t('settings.selectionBg')}
                    value={termTheme.selectionBackground}
                    onChange={setThemeColor('selectionBackground')}
                  />
                  <ColorRow
                    label={t('settings.selectionFg')}
                    value={termTheme.selectionForeground}
                    onChange={setThemeColor('selectionForeground')}
                  />
                </>
              )}
            </div>
            ...
            <div className={`${section} mt-4`}>
              ...
              />
              {openGroups.ansi && <p className={sectionHint}>{t('settings.ansiColorsHint')}</p>}
              {openGroups.ansi && (
                <div className="mt-2 space-y-0">
                  <div className="mb-1 text-xs text-muted-foreground/60">
                    {t('settings.ansiNormal')}
                  </div>
                  {ANSI_NORMAL.map((k) => (
                    <ColorRow
                      key={k}
                      label={ANSI_LABELS[k]}
                      value={termTheme[k]}
                      onChange={setThemeColor(k)}
                    />
                  ))}
                  <div className="mb-1 mt-3 text-xs text-muted-foreground/60">
                    {t('settings.ansiBright')}
                  </div>
                  {ANSI_BRIGHT.map((k) => (
                    <ColorRow
                      key={k}
