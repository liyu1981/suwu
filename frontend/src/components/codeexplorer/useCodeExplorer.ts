...
export function useCodeExplorer(
  ...
  const openPath = useCallback(
    async (rawPath: string, ranges?: HighlightRange[], activateOnLoad = true) => {
      ...
      try {
        ...
        try {
          const res = await authFetch(`/api/file?path=${encodeURIComponent(rawPath)}`, { cache: 'no-store' })
          if (!res.ok) {
  ...
  const reload = useCallback(async (id?: string) => {
    ...
    try {
      const res = await authFetch(`/api/file?path=${encodeURIComponent(tab.path)}`, { cache: 'no-store' })
      if (!res.ok) {
        const body = (await res.json().catch(() => null)) as { error?: string } | null
        throw new Error(body?.error || `HTTP ${res.status}`)
      }
      ...
      setErrors((prev) => ({ ...prev, [tabId]: e instanceof Error ? e.message : i18n.t('codeExplorer.loadFailed') }))
    }
  }, [])
