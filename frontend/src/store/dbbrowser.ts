...
export const saveDBConnectionAtom = atom(null, (get, set, connection: SavedConnection) => {
  ...
  const rest = current.filter((c) => c.id !== connection.id && connectionSignature(c) !== sig);
...
export const deleteDBConnectionAtom = atom(null, (get, set, id: string) => {
  const current = get(dbSavedConnectionsAtom);
