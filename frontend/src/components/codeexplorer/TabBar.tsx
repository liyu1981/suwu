        ...
        {tabs.map((tab) => {
          ...
          return (
            <div
              ...
              title={tab.path}
            >
              <button type="button" onClick={() => onSelect(tab.id)} className="max-w-[12rem] truncate">
                {name}
              </button>
              {isDirty && (
                <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-amber-400" aria-label={t('codeExplorer.unsaved')} />
              )}
              <button
                type="button"
              ...
              >
                <svg className="h-2.5 w-2.5" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round">
