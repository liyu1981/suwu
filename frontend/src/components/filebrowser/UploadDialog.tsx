  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
      <div className="w-[min(90vw,400px)] rounded-2xl border border-white/[0.08] p-5 shadow-2xl menu-glass backdrop-blur-2xl">
        <div className="mb-4 text-base font-semibold tracking-[-0.01em] text-white/80">
          {t('filebrowser.uploadTitle')}
        </div>
        ...
        {!uploading ? (
          <>
            ...
            >
              <svg
                className="mb-2 h-8 w-8 text-white/20 transition-colors group-hover:text-sky-400/50"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="1.5"
              ...
              <span className="text-[11px] text-white/35 transition-colors group-hover:text-white/50">
                {t('filebrowser.clickToSelect')}
              </span>
            </div>
            <input
              ref={fileInputRef}
            ...
            {selectedFiles.length > 0 && (
              <div className="mt-3 max-h-32 overflow-y-auto scrollbar-thin">
                {selectedFiles.map((file) => (
                  <div
                    key={file.name}
                    className="flex items-center justify-between py-1.5 text-sm text-white/60"
                  >
                    <span className="truncate tracking-[-0.01em]">{file.name}</span>
                    <span className="ml-2 shrink-0 text-[11px] tabular-nums text-white/30">
          ...
          <div className="py-4">
            <div className="mb-3 text-[11px] text-white/50">
              {t('filebrowser.uploading')}: {currentFile}
            </div>
            ...
            <div className="mt-1.5 text-right text-[10px] tabular-nums text-white/25">
              {progress}%
            </div>
          </div>
        )}
