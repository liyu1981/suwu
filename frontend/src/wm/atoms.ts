 ...
 */
function migrateLayout(root: LayoutNode | null): {
  ...
  getItem: (key: string): Space[] => {
      ...
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          return parsed.map((s) => {
            ...
            const merged =
              Object.keys(paneData).length > 0 ? { ...existing, ...paneData } : existing;
    ...
    if (legacyRaw !== null) {
      try {
        ...
        return [createSpace('Space 1')].map((s) => ({
          ...s,
          layout,
          paneData: Object.keys(paneData).length > 0 ? paneData : undefined,
        }));
      } catch {
        // ignore
      }
...
export type MenuView = 'menu' | 'shortcuts' | 'settings' | 'appMenu' | 'appSettings' | 'about';
