...
export interface FilePatch { hunks: PatchHunk[]; limited: boolean }

export function comparisonId(repoPath: string, base: string, target: string) {
  return JSON.stringify([repoPath, base, target])
}

export async function comparisonRequest<T>(endpoint: string, params: Record<string, string>, signal: AbortSignal): Promise<T> {
  const response = await authFetch(`/api/git/${endpoint}?${new URLSearchParams(params)}`, { signal })
