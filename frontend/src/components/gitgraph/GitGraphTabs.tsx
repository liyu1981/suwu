...
export function GitGraphTabs({
  ...
  return (
    <div
      ref={ref}
      role="tablist"
      aria-label={t('gitCompare.tabs')}
      className="flex shrink-0 overflow-x-auto border-b border-white/10 glass-control scrollbar-thin"
      onKeyDown={(e) => {
        ...
        const next =
          e.key === 'Home'
            ...
            : e.key === 'End'
              ? all.length - 1
              : (index + (e.key === 'ArrowRight' ? 1 : -1) + all.length) % all.length;
    ...
    >
      {all.map((tab, index) => (
        <div
          key={tab.id}
          className={`flex shrink-0 items-center border-r border-white/10 ${active === tab.id ? 'bg-white/10 text-white' : 'text-white/55'}`}
        >
          <button
            type="button"
            role="tab"
          ...
          {index > 0 && (
            <button
              type="button"
              aria-label={t('gitCompare.closeTab', { label: tab.label })}
              title={t('gitCompare.closeTab', { label: tab.label })}
              className="mr-1 rounded px-1.5 text-xs hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-sky-400"
