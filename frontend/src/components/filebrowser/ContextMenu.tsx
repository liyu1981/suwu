...
export function ContextMenu({
  ...
  if (showNewFolder) {
    return (
      ...
      >
        <div className="mb-2 text-base font-semibold tracking-wide text-white/60">{t('filebrowser.newFolderTitle')}</div>
        <input
          type="text"
          value={newFolderName}
          onChange={(e) => setNewFolderName(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && handleNewFolder()}
          className="mb-3 w-full rounded-lg border border-white/[0.08] bg-white/[0.05] px-3 py-2 text-sm text-white outline-none transition-all duration-150 focus:border-sky-400/40 focus:bg-white/[0.08] focus:ring-1 focus:ring-sky-400/20"
          autoFocus
        />
  ...
  if (showRename) {
    return (
      ...
      >
        <div className="mb-2 text-base font-semibold tracking-wide text-white/60">{t('filebrowser.renameTitle')}</div>
        <input
          type="text"
          value={renameValue}
  ...
  if (showDelete && entry) {
    return (
      ...
      >
        <div className="mb-2 text-base font-semibold tracking-wide text-white/60">{t('filebrowser.deleteTitle')}</div>
        <div className="mb-3 text-sm leading-relaxed text-white/50">
