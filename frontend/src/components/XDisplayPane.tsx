 ...
 */
export default function XDisplayPane({
  display = '99',
  title,
  desktop,
  fps: fpsProp,
  ...
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLCanvasElement>) => {
      ...
      if (
        ['Space', 'Tab', 'Backspace', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(
          e.code,
        )
      ) {
        e.preventDefault();
            ...
            <div className="mb-4 rounded-lg border border-white/5 bg-black/30 p-3">
              {depError.components.map((c) => (
                <div key={c.name} className="flex items-center gap-2 py-1">
                  ...
                  <span
                    className={`text-xs text-white/60 ${c.installed ? 'text-white/60' : 'text-white/90'}`}
                  >
                    {c.name}
            ...
            <div className="mb-4 space-y-2">
              ...
              {Object.entries(depError.install).map(([distro, cmd]) => (
                <div key={distro} className="rounded-md bg-black/40 px-3 py-2">
                  <p className="mb-1 text-xs font-medium text-white/40 capitalize">
                    {distroLabels[distro] ?? distro}
                  </p>
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="mx-4 max-w-sm rounded-xl border border-white/10 bg-gray-900 p-5 shadow-2xl">
            ...
            <p className="mb-4 text-sm text-white/60">
              Display <span className="font-mono text-white/90">{displayInUse.display}</span> is
              already connected by another tile.
            </p>

            <div className="mb-4">
              <label className="mb-1.5 block text-xs font-medium text-white/50">
                Switch to display number:
              </label>
