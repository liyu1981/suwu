...
import {
  CODE_EDITOR_DEFAULTS, CODE_FONT_MIN, CODE_FONT_MAX,
  CODE_LINE_HEIGHT_MIN, CODE_LINE_HEIGHT_MAX, CODE_LINE_HEIGHT_STEP,
...
export function CodeExplorerSettings() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-[11px] leading-relaxed text-muted-foreground">{t('settings.codeExplorerHint')}</p>
      ...
      <div className={section}>
        <label htmlFor="code-editor-font-size" className="text-xs font-medium text-muted-foreground">{t('settings.fontSize')}</label>
        <div className="mt-2 flex items-center gap-3">
          ...
          <input
            ...
            value={settings.fontSize}
            onChange={(event) => {
              if (event.target.value !== '') setSettings({ ...settings, fontSize: event.target.valueAsNumber })
            }}
      ...
      <div className={section}>
        <label htmlFor="code-editor-line-height" className="text-xs font-medium text-muted-foreground">{t('settings.lineHeight')}</label>
        <div className="mt-2 flex items-center gap-3">
          <input
            type="range"
          ...
          <input
            ...
            value={settings.lineHeight}
            onChange={(event) => {
              if (event.target.value !== '') setSettings({ ...settings, lineHeight: event.target.valueAsNumber })
            }}
            className="w-20 rounded border border-white/10 bg-black/30 px-2 py-1 text-sm text-popover-foreground"
          />
