...
function GitGraphContent() {
  ...
  const [selectedWorktree, setSelectedWorktree] = useState<string | null>(
    savedState?.selectedWorktree ?? null,
  );
  ...
  useEffect(() => {
    ...
    const tabs = (savedState.diffTabs ?? []).filter(
      (tab) =>
        typeof tab.repoPath === 'string' &&
        typeof tab.base === 'string' &&
        typeof tab.target === 'string' &&
        tab.id === comparisonId(tab.repoPath, tab.base, tab.target),
    );
    setDiffTabs(tabs);
    setActiveTab(
      tabs.some((tab) => tab.id === savedState.activeTab) ? savedState.activeTab! : 'commits',
    );
    setScrollPosition(savedState.scrollPosition ?? 0);
  }, [savedState]);
  const addDiffTab = useCallback(
    (path: string, base: string, target: string, focusFile?: string) => {
  ...
  const closeDiffTab = useCallback(
    (id: string) => {
      ...
      if (activeTab === id)
        setActiveTab(diffTabs[index + 1]?.id ?? diffTabs[index - 1]?.id ?? 'commits');
  ...
  const fetchWorktrees = useCallback(async () => {
    ...
    try {
      const data = await fetchJson<{ worktrees: GitWorktree[] }>(
        `/api/git/worktrees?path=${encodeURIComponent(repoPath)}`,
      );
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => refreshAll(), autoRefresh);
  ...
  const [dialog, setDialog] = useState<{
    ...
    inputs?: DialogInput[];
    actionLabel?: string;
    onAction: (v: Record<string, string | boolean>) => void;
  } | null>(null);
  ...
  const openCommitDiff = useCallback(
    (commit: GitCommit, focusFile?: string) => {
  ...
  const showCommitContextMenu = useCallback(
    (e: React.MouseEvent, commit: GitCommit) => {
      e.preventDefault();
      e.stopPropagation();
      ...
      if (commit.hash === 'UNCOMMITTED') {
        const items: ContextMenuItem[][] = [
          ...
          [
            {
              label: 'Stash Uncommitted...',
              onClick: () =>
                setDialog({
                  title: 'Stash Changes',
                  message: 'Stash all uncommitted changes?',
                  inputs: [{ type: 'text', name: 'Message', defaultValue: '' }],
                  actionLabel: 'Stash',
                  onAction: (v) => {
                    gitAction('stash', { message: String(v['Message']), includeUntracked: 'true' });
                    refresh();
                  },
                }),
            },
      ...
      const items: ContextMenuItem[][] = [
        [
          {
            label: t('gitCompare.againstParent'),
            onClick: () => setCompareDialog({ commit, mode: 'parent', repoPath: activePath }),
          },
          {
            label: t('gitCompare.againstCustom'),
            onClick: () => setCompareDialog({ commit, mode: 'custom', repoPath: activePath }),
          },
        ],
        [
          {
            label: 'Checkout',
            onClick: () => {
              gitAction('checkout-commit', { hash: commit.hash });
              refresh();
            },
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Cherry Pick',
                message: `Cherry pick <b>${commit.hash.slice(0, 7)}</b>?`,
                inputs: [{ type: 'checkbox', name: 'No Commit', checked: false }],
                actionLabel: 'Cherry Pick',
                onAction: (v) => {
                  gitAction('cherry-pick', { hash: commit.hash, noCommit: String(v['No Commit']) });
                  refresh();
                },
              }),
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Revert',
                message: `Revert commit <b>${commit.hash.slice(0, 7)}</b>?`,
                actionLabel: 'Revert',
                onAction: () => {
                  gitAction('revert', { hash: commit.hash });
                  refresh();
                },
              }),
          },
        ...
        [
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Create Branch',
                message: `Create a new branch at <b>${commit.hash.slice(0, 7)}</b>:`,
                inputs: [{ type: 'text', name: 'Branch name', defaultValue: '' }],
                actionLabel: 'Create',
                onAction: (v) => {
                  const name = String(v['Branch name']).trim();
                  if (name) {
                    gitAction('create-branch', { name, startPoint: commit.hash });
                    refresh();
                  }
                },
              }),
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Add Tag',
                message: `Add tag at <b>${commit.hash.slice(0, 7)}</b>:`,
                inputs: [
                  { type: 'text', name: 'Tag name', defaultValue: '' },
                  { type: 'text', name: 'Message', defaultValue: '' },
                ],
                actionLabel: 'Create Tag',
                onAction: (v) => {
                  const name = String(v['Tag name']).trim();
                  if (name) {
                    gitAction('add-tag', {
                      name,
                      hash: commit.hash,
        ...
        [
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Reset',
                message: `Reset current branch to <b>${commit.hash.slice(0, 7)}</b>?`,
                inputs: [
                  {
                    type: 'radio',
                    name: 'Mode',
                    options: [
                      { label: 'Soft — keep changes, reset head', value: 'soft' },
                      { label: 'Mixed — keep working tree, reset index', value: 'mixed' },
                ...
                actionLabel: 'Reset',
                onAction: (v) => {
                  gitAction('reset-to-commit', { hash: commit.hash, mode: String(v['Mode']) });
                  refresh();
                },
              }),
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Merge',
                message: `Merge <b>${commit.hash.slice(0, 7)}</b> into current branch?`,
                actionLabel: 'Merge',
                onAction: () => {
                  gitAction('merge', { ref: commit.hash });
                  refresh();
                },
              }),
          },
          {
            label: 'Rebase on Commit...',
            onClick: () =>
              setDialog({
                title: 'Rebase',
                message: `Rebase current branch onto <b>${commit.hash.slice(0, 7)}</b>?`,
                actionLabel: 'Rebase',
                onAction: () => {
                  gitAction('rebase', { upstream: commit.hash });
                  refresh();
                },
              }),
          },
  ...
  const showBranchContextMenu = useCallback(
        ...
        [
          {
            ...
            disabled: isCurrent,
            onClick: () => {
              gitAction('checkout-branch', { branch: branchName });
              refresh();
            },
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Rename Branch',
                message: `Rename <b>${branchName}</b> to:`,
                inputs: [{ type: 'text', name: 'New name', defaultValue: branchName }],
                actionLabel: 'Rename',
                onAction: (v) => {
                  const name = String(v['New name']).trim();
                  if (name && name !== branchName) {
                    gitAction('rename-branch', { oldName: branchName, newName: name });
                    refresh();
                  }
                },
              }),
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Delete Branch',
                message: `Delete branch <b>${branchName}</b>?`,
                inputs: [{ type: 'checkbox', name: 'Force delete', checked: false }],
                actionLabel: 'Delete',
                onAction: (v) => {
                  gitAction('delete-branch', {
                    branch: branchName,
                    force: String(v['Force delete']),
                  });
                  refresh();
                },
              }),
        ...
        [
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Merge',
                message: `Merge <b>${branchName}</b> into current branch?`,
                actionLabel: 'Merge',
                onAction: () => {
                  gitAction('merge', { ref: branchName });
                  refresh();
                },
              }),
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Rebase',
                message: `Rebase current branch onto <b>${branchName}</b>?`,
                actionLabel: 'Rebase',
                onAction: () => {
                  gitAction('rebase', { upstream: branchName });
                  refresh();
                },
              }),
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Push Branch',
                message: `Push <b>${branchName}</b> to remote?`,
                inputs: [
                  { type: 'checkbox', name: 'Set Upstream', checked: true },
                  {
                    type: 'radio',
                    name: 'Mode',
                    options: [
                      { label: 'Normal', value: 'normal' },
                      { label: 'Force With Lease', value: 'forceWithLease' },
                ...
                actionLabel: 'Push',
                onAction: (v) => {
                  gitAction('push-branch', {
                    branch: branchName,
                    setUpstream: String(v['Set Upstream']),
                    force: v['Mode'] === 'force' ? 'true' : 'false',
                    forceWithLease: v['Mode'] === 'forceWithLease' ? 'true' : 'false',
                  });
                  refresh();
                },
              }),
  ...
  const showTagContextMenu = useCallback(
    (e: React.MouseEvent, tag: { name: string; annotated: boolean }) => {
      e.preventDefault();
      const zoom = parseFloat(document.documentElement.style.zoom) || 1;
      const items: ContextMenuItem[][] = [
        [
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Delete Tag',
                message: `Delete tag <b>${tag.name}</b>?`,
                actionLabel: 'Delete',
                onAction: () => {
                  gitAction('delete-tag', { name: tag.name });
                  refresh();
                },
              }),
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Push Tag',
                message: `Push tag <b>${tag.name}</b> to remote?`,
                actionLabel: 'Push',
                onAction: () => {
                  gitAction('push-tag', { name: tag.name });
                  refresh();
                },
              }),
          },
  ...
  const showStashContextMenu = useCallback(
    (e: React.MouseEvent, stash: { selector: string; baseHash: string }) => {
      e.preventDefault();
      const zoom = parseFloat(document.documentElement.style.zoom) || 1;
      const items: ContextMenuItem[][] = [
        [
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Apply Stash',
                message: `Apply stash <b>${stash.selector}</b>?`,
                inputs: [{ type: 'checkbox', name: 'Reinstate Index', checked: false }],
                actionLabel: 'Apply',
                onAction: (v) => {
                  gitAction('stash-apply', {
                    selector: stash.selector,
                    reinstateIndex: String(v['Reinstate Index']),
                  });
                  refresh();
                },
              }),
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Pop Stash',
                message: `Pop stash <b>${stash.selector}</b>?`,
                inputs: [{ type: 'checkbox', name: 'Reinstate Index', checked: false }],
                actionLabel: 'Pop',
                onAction: (v) => {
                  gitAction('stash-pop', {
          ...
          },
          {
            ...
            onClick: () =>
              setDialog({
                title: 'Drop Stash',
                message: `Drop stash <b>${stash.selector}</b>?`,
                actionLabel: 'Drop',
                onAction: () => {
                  gitAction('stash-drop', { selector: stash.selector });
                  refresh();
                },
              }),
          },
        ...
        [
          {
            ...
            onClick: () =>
              setDialog({
                ...
                message: `Create a branch from stash <b>${stash.selector}</b>:`,
                inputs: [{ type: 'text', name: 'Branch name', defaultValue: '' }],
                actionLabel: 'Create',
                onAction: (v) => {
                  const name = String(v['Branch name']).trim();
                  if (name) {
                    gitAction('branch-from-stash', { branch: name, selector: stash.selector });
                    refresh();
                  }
                },
              }),
  ...
  const handleCommitClick = useCallback((_commit: GitCommit, index: number) => {
  ...
  return (
    <>
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <div className="flex h-8 shrink-0 items-center gap-1 rounded-t-[6px] border-b border-white/10 px-3 py-1.5 glass-control">
          <button
            type="button"
            onClick={refreshAll}
            className={`grid h-5 w-5 place-items-center rounded transition hover:bg-white/10 ${autoRefresh > 0 ? 'text-green-400' : 'text-white/50 hover:text-white/70'}`}
            title="Refresh"
          >
            <RefreshIcon />
          </button>
          ...
          {repoPath ? (
            <>
              <button
                ...
                className="flex min-w-0 flex-1 items-center gap-1.5 rounded-md bg-white/[0.04] px-2 py-1 text-left text-white/50 transition-all duration-150 hover:bg-white/[0.08] hover:text-white/70"
              >
                <svg
                ...
                <span className="shrink-0 text-white/30">
                  {copied ? (
                    <svg className="h-3 w-3 text-green-400" viewBox="0 0 16 16" fill="currentColor">
              ...
              <button
                ...
                className={`shrink-0 flex items-center gap-1 rounded-md px-2 py-1 text-xs transition-all duration-150 active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-30 ${
                ...
                title={
                  selectedWorktree !== null
                    ? 'All branches is not available in worktree diff mode'
                    : allBranches
                      ? 'Showing all branches — click to show current branch only'
                      : 'Show all branches'
                }
              >
              ...
              {worktrees.length > 1 && (
                ...
                >
                  ...
                  <svg
                    className={`h-2.5 w-2.5 shrink-0 opacity-50 transition-transform ${showWorktreeBrowser ? 'rotate-180' : ''}`}
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
              ...
              <span className="shrink-0 rounded-md bg-white/[0.04] px-2 py-1 text-[10px] text-white/50">
                {error ? '—' : `${commits.length} commits`}
              </span>
              <button
                type="button"
                onClick={() => {
                ...
                className="shrink-0 rounded-md bg-white/[0.06] px-2.5 py-1 text-xs text-white/70 transition-all duration-150 hover:bg-white/[0.12] hover:text-white active:scale-[0.97]"
              >
                Change repo
              </button>
            </>
        ...
        <div
          id="git-panel-0"
          role={diffTabs.length ? 'tabpanel' : undefined}
          aria-labelledby={diffTabs.length ? 'git-tab-0' : undefined}
        ...
        >
          ...
          {showWorktreeBrowser && worktrees.length > 1 && (
            <div className="flex flex-1 flex-col overflow-auto rounded-b-[6px] border-x border-b border-white/[0.10] bg-black/20 scrollbar-thin">
              <div className="flex shrink-0 items-center border-b border-white/[0.06] bg-white/[0.03] px-3 py-2 text-xs font-semibold uppercase tracking-widest text-white/35">
              ...
              {worktrees.map((wt) => (
                <button
                  ...
                  className={`flex w-full items-center px-3 py-2 text-sm text-left transition-colors ${
                    (wt.isMain && selectedWorktree === null) ||
                    (!wt.isMain && selectedWorktree === wt.path)
                ...
                >
                  <div className="flex flex-1 min-w-0 items-center gap-1.5">
                    <span
                      className={`inline-block h-2 w-2 shrink-0 rounded-full ${wt.isMain ? 'bg-sky-400' : 'bg-white/30'}`}
                    />
                  ...
                  <span className="w-24 shrink-0 text-white/40">
                    {wt.isMain ? '—' : (mainWorktree?.branch ?? 'main')}
                  </span>
                  <span className="w-32 shrink-0 text-white/40">
                    {wt.lastActive > 0
                      ? new Date(wt.lastActive).toLocaleDateString(undefined, {
                          month: 'short',
                          day: 'numeric',
                          year: 'numeric',
                        })
                  ...
                  <div className="w-20 shrink-0 flex justify-end gap-2 text-[10px]">
          ...
          {!showWorktreeBrowser && (picking || repoPath === null) ? (
            <div
              className="flex flex-1 flex-col overflow-y-auto scrollbar-thin rounded-b-[6px] border-x border-b border-x-white/[0.10] border-b-white/[0.10] bg-black/20 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]"
              style={{
                maxHeight: `calc(${100 / (parseFloat(document.documentElement.style.zoom) || 1)}vh - 90px)`,
              }}
            >
            ...
            <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-b-[6px] border-x border-b border-white/[0.10] bg-black/20 p-6">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={handleRetry}
                  className="rounded-md bg-white/10 px-3 py-1.5 text-xs text-white/80 transition hover:bg-white/20 hover:text-white"
                >
                  Retry
                </button>
                <button
                  ...
                  className="rounded-md bg-green-500/20 px-3 py-1.5 text-xs font-medium text-green-300 transition hover:bg-green-500/30"
                >
            ...
            <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-b-[6px] border-x border-b border-white/[0.10] bg-black/20 p-6">
              <div className="max-w-sm text-center text-[11px] text-white/60">
            ...
            !showWorktreeBrowser && (
              <div
                className="min-h-0 flex-1 overflow-auto rounded-b-[6px] border-x border-b border-x-white/[0.10] border-b-white/[0.10] bg-black/20 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] scrollbar-thin"
                ...
                <div className="flex">
                  <div className="sticky left-0 z-10 shrink-0" style={{ backgroundColor: bgColor }}>
                    <GraphRenderer
                      ...
                      expandedCommitIndex={expandedIndex ?? -1}
                  ...
                  <div className="min-w-0 flex-1">
                    {commits.map((commit, index) => (
                      <div
                        key={commit.hash}
                        data-commit-idx={index}
                        style={{ minHeight: ROW_HEIGHT }}
                      >
                        <CommitRow
                          commit={commit}
                          branchColors={graphLayout?.branchColors}
                          isExpanded={expandedIndex === index}
                          onClick={() => handleCommitClick(commit, index)}
                          onContextMenu={showCommitContextMenu}
                          onBranchContextMenu={showBranchContextMenu}
                          onTagContextMenu={showTagContextMenu}
                          onStashContextMenu={showStashContextMenu}
                        />
                        {expandedIndex === index && repoPath && (
                          <ExpandedCommitRow
                            repoPath={activePath}
                            hash={commit.hash}
                            height={DETAILS_HEIGHT}
                            onParentClick={handleParentClick}
                            onOpenDiff={(focusFile) => openCommitDiff(commit, focusFile)}
                          />
                        )}
                      </div>
                    ))}
                    ...
                    {!loading && !hasMore && commits.length > 0 && (
                      <div className="py-3 text-center text-[11px] text-white/20">
                        All commits loaded ({commits.length})
                      </div>
                    )}
                  </div>
                </div>
        ...
        {diffTabs.map((tab, index) => (
          <div
            key={tab.id}
            id={`git-panel-${index + 1}`}
            role="tabpanel"
            aria-labelledby={`git-tab-${index + 1}`}
            className="min-h-0 flex-1 flex-col overflow-hidden"
          ...
          >
            <CommitDiffTab
              ...
              onSwap={() => addDiffTab(tab.repoPath, tab.target, tab.base)}
              onParent={(base) => addDiffTab(tab.repoPath, base, tab.target)}
            />
          </div>
        ))}
      </div>
      ...
      {compareDialog && (
        <CompareCommitDialog
          commit={compareDialog.commit}
          commits={commits}
          repoPath={compareDialog.repoPath}
          mode={compareDialog.mode}
          onCancel={() => setCompareDialog(null)}
          onOpen={(comparison) => {
            addDiffTab(
              compareDialog.repoPath,
              comparison.base,
              comparison.target,
              compareDialog.focusFile,
            );
            setCompareDialog(null);
          }}
        />
...
function CommitRow({
  ...
  isExpanded: boolean;
  onClick: () => void;
  onContextMenu: (e: React.MouseEvent, c: GitCommit) => void;
  onBranchContextMenu: (e: React.MouseEvent, branch: string) => void;
  onTagContextMenu: (e: React.MouseEvent, tag: { name: string; annotated: boolean }) => void;
  onStashContextMenu: (e: React.MouseEvent, stash: { selector: string; baseHash: string }) => void;
}) {
  const date = new Date(commit.date);
  const isUncommitted = commit.hash === 'UNCOMMITTED';
  return (
    <div
      className={`flex h-6 cursor-pointer items-center leading-none transition-colors ${isExpanded ? 'bg-white/10' : isUncommitted ? 'bg-orange-500/5 hover:bg-orange-500/10' : 'hover:bg-white/5'}`}
      style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}
      onClick={onClick}
      onContextMenu={(e) => onContextMenu(e, commit)}
    >
      {isUncommitted ? (
        <div className="w-20 shrink-0 pl-2 text-[10px] text-orange-400/70">now</div>
      ) : (
        <div className="w-20 shrink-0 pl-2 text-[10px] text-white/50">
          {date.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
        </div>
      )}
      <div className="min-w-0 flex-1 truncate text-sm text-white/90">{commit.message}</div>
      <div className="ml-2 flex shrink-0 items-center gap-1">
        {(commit.heads || []).map((h) => (
          <span
            ...
            onContextMenu={(e) => {
              e.stopPropagation();
              onBranchContextMenu(e, h);
            }}
            style={{
              backgroundColor: (branchColors?.[h] ?? '#3b82f6') + '33',
              color: branchColors?.[h] ?? '#60a5fa',
            }}
            className="cursor-pointer rounded-full px-1.5 py-[1px] text-[10px] transition-[filter] hover:brightness-125"
          >
            {h}
          </span>
        ))}
        {(commit.tags || []).map((t) => (
          <span
            ...
            onContextMenu={(e) => {
              e.stopPropagation();
              onTagContextMenu(e, t);
            }}
            className="cursor-pointer rounded-full bg-yellow-500/20 px-1.5 py-[1px] text-[10px] text-yellow-400 hover:bg-yellow-500/30"
          >
            {t.name}
          </span>
        ))}
        {commit.stash && (
          <span
            onContextMenu={(e) => {
              e.stopPropagation();
              onStashContextMenu(e, commit.stash!);
            }}
            className="cursor-pointer rounded-full bg-purple-500/20 px-1.5 py-[1px] text-[10px] text-purple-400 hover:bg-purple-500/30"
          >
            {commit.stash.selector}
          </span>
        )}
      </div>
      <div className="ml-2 w-24 shrink-0 truncate text-[11px] text-white/40">{commit.author}</div>
      <div className="ml-2 w-14 shrink-0 pr-2 text-right text-[10px] font-mono text-white/40">
...
function ExpandedCommitRow({
  ...
  useEffect(() => {
    ...
    fetchJson<GitCommitDetails>(
      `/api/git/commit?path=${encodeURIComponent(repoPath)}&hash=${encodeURIComponent(hash)}`,
    )
      .then((d) => {
        if (!cancelled) setDetails(d);
      })
      .catch((err) => {
        if (!cancelled) setLoadError(err instanceof Error ? err.message : 'Failed');
      })
      .finally(() => {
  ...
  return (
    <div
      className="flex overflow-y-auto scrollbar-thin border-b border-white/[0.06] bg-white/[0.03]"
      style={{ height }}
    ...
    >
      {loading ? (
        ...
        <div className="flex w-full items-center justify-center px-4 text-[11px] text-red-300">
          {loadError || 'Failed to load commit details'}
        </div>
      ) : (
        <>
          {/* ── Left: commit metadata ──────────────────────────────── */}
          <div className="flex w-52 shrink-0 flex-col gap-1 border-r border-white/[0.06] px-3 py-2.5 text-xs leading-relaxed">
            {/* Hash + copy */}
            <button
              type="button"
              onClick={copyHash}
              className="group flex items-center gap-1.5 rounded px-1.5 py-0.5 -mx-1.5 text-left transition-colors hover:bg-white/[0.06]"
              title="Copy full commit hash"
            >
              <svg
                className="h-3 w-3 shrink-0 text-white/30 transition-colors group-hover:text-white/60"
                viewBox="0 0 16 16"
                fill="currentColor"
              >
                <path d="M0 6.75C0 5.784.784 5 1.75 5h1.5a.75.75 0 0 1 0 1.5h-1.5a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-1.5a.75.75 0 0 1 1.5 0v1.5A1.75 1.75 0 0 1 9.25 16h-7.5A1.75 1.75 0 0 1 0 14.25v-7.5z" />
              ...
              <span className="shrink-0 text-white/20">
                {hashCopied ? <span className="text-green-400">copied!</span> : 'copy'}
              </span>
            </button>
            ...
            <div className="mt-1 border-t border-white/[0.06] pt-1">
              ...
              <p className="mt-0.5 whitespace-pre-wrap text-sm leading-relaxed text-white/80">
                {details.message}
              </p>
              {details.body && details.body !== details.message && (
                <p className="mt-1 whitespace-pre-wrap text-sm leading-relaxed text-white/50">
                  {details.body}
                </p>
              )}
            </div>
            ...
            <div className="mt-auto flex gap-2 border-t border-white/[0.06] pt-1.5">
              <span className="font-mono text-green-400">
                +{(details.fileChanges ?? []).reduce((s, f) => s + (f.adds || 0), 0)}
              </span>
              <span className="font-mono text-red-400">
                −{(details.fileChanges ?? []).reduce((s, f) => s + (f.dels || 0), 0)}
              </span>
            </div>
          </div>
          ...
          <div className="min-w-0 flex-1 overflow-y-auto scrollbar-thin px-2 py-2">
            {onOpenDiff && (
              <button
                type="button"
                onClick={() => onOpenDiff()}
                className="mb-2 rounded bg-sky-500/15 px-2 py-1 text-xs text-sky-300 hover:bg-sky-500/25"
              >
                {t('gitCompare.openSplit')}
              </button>
            )}
            {(details.fileChanges ?? []).length === 0 ? (
              ...
              (details.fileChanges ?? []).map((f) => (
                <FileChangeItem
                  ...
                  repoPath={repoPath}
                  hash={hash}
                  onOpenDiff={onOpenDiff ? () => onOpenDiff(f.newPath) : undefined}
                  open={openDiff === f.newPath}
                  onToggle={() => setOpenDiff(openDiff === f.newPath ? null : f.newPath)}
                />
              ))
            )}
          </div>
...
const CHANGE_STYLES: Record<string, string> = {
  ...
  R: 'bg-violet-500/20 text-violet-400',
  U: 'bg-cyan-500/20 text-cyan-400',
};
...
function FileChangeItem({
  ...
  useEffect(() => {
    ...
    fetchJson<{ diff: string }>(`/api/git/diff?${p}`)
      ...
      .catch((err) => {
        if (!cancelled) setDiffError(err instanceof Error ? err.message : 'Failed');
      })
      .finally(() => {
  ...
  return (
    <div className="mb-1">
      <button
        type="button"
        onClick={onToggle}
        className={`flex w-full items-center gap-2 rounded px-2 py-1 text-sm text-left transition hover:bg-white/[0.06] ${open ? 'bg-white/[0.06]' : ''}`}
      >
        <span
          className={`w-4 shrink-0 rounded text-center text-[10px] font-mono font-bold ${CHANGE_STYLES[change.type] ?? CHANGE_STYLES.M}`}
        >
          {change.type}
        </span>
        <span className="min-w-0 flex-1 truncate font-mono tracking-tight text-white/75 hover:text-white">
          {change.newPath}
          {change.type === 'R' && change.oldPath !== change.newPath && (
            <span className="text-white/35"> ← {change.oldPath}</span>
          )}
        </span>
        {change.adds || change.dels ? (
          <span className="shrink-0 text-[10px] font-mono">
            <span className="text-green-400">+{change.adds}</span>{' '}
            <span className="text-red-400">−{change.dels}</span>
          </span>
        ) : null}
        <svg
          className={`h-3 w-3 shrink-0 text-white/30 transition-transform duration-150 ${open ? 'rotate-90' : ''}`}
          viewBox="0 0 16 16"
          fill="currentColor"
        >
      ...
      {open && onOpenDiff && (
        <button
          type="button"
          onClick={onOpenDiff}
          className="ml-6 rounded px-2 py-1 text-xs text-sky-300 hover:bg-white/5"
        >
          {t('gitCompare.openSplit')}
        </button>
      )}
      {open && (
        <div className="ml-6 mt-0.5 overflow-hidden rounded border border-white/[0.08] bg-black/40">
          {diffLoading ? (
            ...
            <div className="p-2.5 text-[11px] text-red-300">{diffError}</div>
          ) : diff ? (
            <DiffView diff={diff} />
          ) : (
...
function DiffView({ diff }: { diff: string }) {
  return (
    <pre className="max-h-40 overflow-auto scrollbar-thin p-2 text-xs font-mono leading-[1.5]">
      {diff.split('\n').map((line, i) => {
