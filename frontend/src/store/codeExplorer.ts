...
export function normalizeCodeEditorSettings(value: unknown): CodeEditorSettings {
  ...
  return {
    fontFamily:
      family && family.length <= 512 && !/[\x00-\x1f\x7f]/.test(family)
        ? family
        : CODE_EDITOR_DEFAULTS.fontFamily,
    fontSize:
    ...
    lineHeight:
      typeof input.lineHeight === 'number' && Number.isFinite(input.lineHeight)
        ? Math.min(
            ...
            Math.max(CODE_LINE_HEIGHT_MIN, Math.round(input.lineHeight * 10) / 10),
          )
...
const storedCodeEditorSettingsAtom = atomWithStorage<unknown>(
  'suwu.code-editor-settings',
...
export const codeEditorSettingsAtom = atom(
  ...
  (_get, set, value: CodeEditorSettings) =>
    set(storedCodeEditorSettingsAtom, normalizeCodeEditorSettings(value)),
);
